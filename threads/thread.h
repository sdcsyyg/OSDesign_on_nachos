...
class Thread {
  ...
  public:
	// initialize a Thread 
    Thread(char* debugName, int staticPriority_p = 0);
	...
	{
		if (increment < 20 && increment > -20 && increment != 0)
  ...
  private:
	...
	int priority;		//调度依据的优先级
	int staticPriority;	//静态优先级
	int dynamicPriority;//动态优先级
